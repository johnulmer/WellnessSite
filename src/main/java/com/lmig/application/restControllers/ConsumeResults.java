...
public class ConsumeResults {
	...
	public WellnessEvent getLngLatFromGoogle(WellnessEvent addr) {
		...
		String fullAddress = addr.getStreetAddress() + " " + addr.getCity() + " , " + addr.getState() + " , "
				+ addr.getZipCode();
		...
		try {
			...
			Double lat = jobj.get("geometry").getAsJsonObject().get("location").getAsJsonObject().get("lat")
					.getAsDouble();
			Double lng = jobj.get("geometry").getAsJsonObject().get("location").getAsJsonObject().get("lng")
					.getAsDouble();
		...
		return addr;
		// return output.toString(); // This returned String is JSON string from which
		// you can retrieve all key value pair and can save it in POJO.
	}
}