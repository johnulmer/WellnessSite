...
/**
 * This just adds our intercepter to the registry so that it is used on the
 * path(s) we specify. In our case we are adding to any path that starts with
 * /api/ (note I created the login controller on /open/login so it will not be
 * bothered by this intercepter which is good because you have to be able to get
 * there without logging in
 */
@Configuration
